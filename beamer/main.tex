...
\begin{frame}{Le merge : le fast-forward}
	\begin{figure}[H]
		\only<1>{
		\includegraphics[height=6cm]{images/3-collaboration/ff.eps}
		\caption{\texttt{git merge avec no-ff}}
		}
		\only<2>{
		\includegraphics[height=6cm]{images/3-collaboration/ff2.eps}
		\caption{\texttt{git merge sans no-ff}}
		}
	\end{figure}
\end{frame}
...
\begin{frame}{Le merge : le squash}
	\begin{figure}[H]
		\only<1>{
		\includegraphics[height=6cm]{images/3-collaboration/merge.eps}
		\caption{\texttt{git merge}}
		}
		\only<2>{
		\includegraphics[height=6cm]{images/3-collaboration/mergesquash.eps}
		\caption{\texttt{git merge --squash}}
		}
	\end{figure}
\end{frame}
...
\begin{frame}{Le rebase : on veut un historique linéaire}
	\begin{figure}[H]
		...
		\only<2>{
		\includegraphics[height=6cm]{images/3-collaboration/rebase.eps}
...
\begin{frame}{Le {pull} devrait être interdit…}
	...
	\end{itemize}
	\lstinputlisting[language=sh, caption=Un git pull si on est sur master]{pull}%
	}
	\begin{itemize}
	...
	\end{itemize}
	\uncover<4->{
	\lstinputlisting[language=sh, caption=Se mettre à jour si on est sur master]{fetch}%
	}
\end{frame}
...
\end{frame}
\begin{frame}{Le rebase interactif : réécrire l'histoire}
	\lstinputlisting[language=sh, caption=Utilisation du rebase interactif]{rebase-interactif}%
	\begin{itemize}
...
\begin{frame}{Bisect : trouver d'où vient le bug}
	...
	\end{itemize}
	\lstinputlisting[language=sh, caption=Utilisation de git bisect]{bisect}%
	}
	\uncover<2>{
	\begin{itemize}
	...
	\lstinputlisting[language=sh, caption=Utilisation de git bisect pour lancer les tests]{bisect2}%
	}
